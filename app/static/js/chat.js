...
function selectChat(item){
	...
	notifications.emit("get_messages", {id : uid})
}
...
function sendMessage(event){
	...
	notifications.emit("message_send", msg, id)
}
...
notifications.on("show_chat", function({messages, chat_id}){
	setChatActive(chat_id)
	document.querySelector(".chat_space>.messsages").classList.remove("empty")
	displayChatMessages(messages)
	notifications.emit("read", chat_id=getActiveID())
})

notifications.on("chat_pending", function({chat_id}){
	setChatActive(chat_id)
...
notifications.on("message_get", function({message}){
	hist = document.querySelector(".chat_space>.messsages>.history")
...
notifications.on("unread_chats", function(unread){
	unread.forEach(q => {
